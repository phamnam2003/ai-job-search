...
\begin{itemize}
    \item Architected the system: database modeling and backend stack decisions for an AI document-summarization and task-generation platform, now being positioned for customers beyond C06.
    \item Implemented realtime client updates over WebSocket and SSE, with background workers and cron jobs for recurring and long-running processing; deployed to Kubernetes with ArgoCD.
...
\begin{itemize}
    \item gRPC reference built from the protocol up (all four method types, interceptors, TLS/mTLS); \texttt{challenges} repo with 20 Gang-of-Four patterns in Go; \texttt{go-2fa} TOTP implementation.
\end{itemize}}}
